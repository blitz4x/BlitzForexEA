...
#import "Wininet.dll"
   ...
   int  InternetReadFile   (int,uchar &sBuffer[],int ,int &nRead[]);
   int  InternetReadFileExW(int, string, int, int);
...
string   lastMsg    = "",  check = "";
string   clientURL  =  "https://blitz4x.com/wp-blitz4x/blitz.php";
...
void OnTick()
{
   ...
   if (TimeCurrent() > nextRes)
   {
      nextRes = TimeCurrent();
      check = "----------\n" + WindowExpertName() + "\nLocal time " + TimeToStr(TimeLocal(), TIME_DATE | TIME_SECONDS);

      int HttpOpen    = InternetOpenW   ("UZI_HTTP", 0, NULL, NULL, 0);
      int HttpConnect = InternetConnectW(HttpOpen, "127.0.0.1", 80, NULL, NULL, 3, 0, 1);
      int HttpRequest = InternetOpenUrlW(HttpOpen, clientURL + "?up=" + IntegerToString(Seconds()), NULL, 0, 0, 0);

      int    nRead[1];
      uchar  sBuffer[5120];
      string res = "";

      bool bResult = InternetReadFile(HttpRequest, sBuffer, ArraySize(sBuffer), nRead);
      res = CharArrayToString(sBuffer, 0, nRead[0], CP_UTF8);
